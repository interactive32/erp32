...
if (!class_exists('TCPDF', false)) {
	...
	class TCPDF {
		...
		public function __construct($orientation='P', $unit='mm', $format='A4', $unicode=true, $encoding='UTF-8', $diskcache=false) {
			...
			// alcal: set timestamp
			$this->timestamp = $this->l['w_datetime'].' '.date('d/m/Y H:i:s');
			// set disk caching
		 ...
		 * @access public
		 */
		public function setHeaderData($ln='', $lw=0, $ht='', $hs='', $hsub='') {
			$this->header_logo = $ln;
			$this->header_logo_width = $lw;
			$this->header_title = $ht;
			$this->header_string = $hs;
			$this->header_subject = $hsub;			
		}
		...
		public function getHeaderData() {
			...
			$ret['subject'] = $this->header_subject;			
			return $ret;
		}

		/*
		 * alcal: extra header - for report title
		 */
		public function setHeaderExtraData($extra='') {

			$this->header_extra = $extra;
		}
	
		
		public function getHeaderExtraData() {
			$ret= $this->header_extra;
			return $ret;
		}
		...
		public function Header() {
			...
			$headerextradata = $this->getHeaderExtraData();
			...
			{
			// header subject
			$this->Ln(2); // small spacer
			$this->SetFont($headerfont[0], $headerfont[1], $headerfont[2] + 1);
			$this->SetX($header_x);
			$this->MultiCell(0, $cell_height, $headerdata['subject'], 0, 'L', 0, 1, '', '', true, 0, false);
			}			
			...
			$this->Cell(0, 0, $headerextradata, '', 0, 'C');
			
			if ($this->getFilterName())
			{			
				$this->Ln();
				$this->SetFont($headerfont[0], 'B', 8);			
				$this->Cell(0, 5, '( '.$this->getFilterName().' )', '', 0, 'C');
			}
		...
		public function Footer() {
			...
			// show timestamp
			//$this->Cell(40, 0, $this->timestamp, 'T', 0, 'R');
			
			$this->Cell(40, 5, 'www.erp32.com', 'T', 0, 'R');			
			...
			if ($this->getRTL()) {
				...
				$this->SetX($ormargins['left']);
				$this->Cell(0, 5, $pagenumtxt, 'T', 0, 'R');
			}
		}
		...
		protected function _getTrueTypeFontSubset($font, $subsetchars) {
			...
			// TODO Alcal: Load font from file	
			/*
			unset($font);
			$fp=fopen('c:/font.fff','r');
			$zz = fread($fp,filesize('c:/font.fff'));
			fclose($fp);
		...
		protected function _putfonts() {
				...
				if (!$this->empty_string($fontfile)) {
					...
					if ((!$compressed) AND (isset($info['length2']))) {
						...
						if (!file_exists($fontfile.'.cached')){
							...
							$subsetchars = array_fill(0, 500, true); // subset of all chars 0-512
			...
			$fp=fopen('c:/font1.fff','r');
			$zz = fread($fp,filesize('c:/font1.fff'));
			fclose($fp);
			//var_dump(unserialize($zz));
